...
function New-RegistryBrowserView {
    ...
    $navigateToPath = {
        ...
        if (-not $rootNode) {
            & $showMessage -Title 'Navigation' -Message "Invalid registry path: $TargetPath"
            return
        }
        ...
        for ($i = 1; $i -lt $segments.Count; $i++) {
            ...
            if (-not $found) {
                & $showMessage -Title 'Navigation' -Message "Key not found: $seg`n`nPath: $TargetPath"
                return
            }
    ...
    $addCurrentToFavorites = {
        ...
        if ([string]::IsNullOrWhiteSpace($path)) {
            & $showMessage -Title 'Favorites' -Message 'Select a key first.'
            return
        }
        ...
        if ($favs | Where-Object { $_.Name -eq $name }) {
            if (-not (& $confirm -Title 'Favorites' -Message "A favorite named '$name' already exists. Replace it?")) { return }
    ...
    $mnuTreeNewKey.Add_Click({
        ...
        try {
            ...
            if ($null -eq $regKey) {
                & $showMessage -Title 'Error' -Message 'Cannot open key for writing.'
                return
            }
            ...
            & $showMessage -Title 'Error' -Message "Failed to create key: $($_.Exception.Message)"
        }
        finally {
            $parts = $path -split '\\', 2
            if ($null -ne $regKey -and $parts.Count -gt 1) { $regKey.Dispose() }
    ...
    $mnuTreeDeleteKey.Add_Click({
        ...
        if (-not (& $confirm -Title 'Confirm Delete' -Message "Delete key '$($sel.Header)' and all its subkeys?`n`n$path")) { return }
        ...
        try {
            ...
            if ($null -eq $parentKey) {
                & $showMessage -Title 'Error' -Message 'Cannot open parent for writing.'
                return
            }
            ...
            & $showMessage -Title 'Error' -Message "Failed to delete: $($_.Exception.Message)"
        }
        finally {
    ...
    $mnuTreeExport.Add_Click({
        ...
        try {
            ...
            & $showMessage -Title 'Export' -Message "Export failed: $($_.Exception.Message)"
    ...
    $mnuTreeImport.Add_Click({
        ...
        $path = [string]$sel.Tag
        if (-not (& $isWritable $path)) {
            & $showMessage -Title 'Import' -Message 'Import is only allowed into HKCU.'
            return
        }
        ...
        try { $parsed = & $parseRegFile $dlg.FileName }
            ...
            & $showMessage -Title 'Import' -Message "Parse failed: $($_.Exception.Message)"
            return
        }
        if (-not $parsed -or $parsed.Ops.Count -eq 0) {
            & $showMessage -Title 'Import' -Message 'No operations found in file.'
            return
        }
        ...
        if ($nonHkcu.Count -gt 0) {
            ...
            & $showMessage -Title 'Import' -Message "Import refused. File contains $($nonHkcu.Count) operation(s) outside HKEY_CURRENT_USER:`r`n`r`n$sample$(if ($nonHkcu.Count -gt 5) { "`r`n  ..." })"
    ...
    $openValueEditor = {
        ...
        $result = & $showRegistryValueEditorSb -Owner $window -ValueName $ValueName -Kind $Kind `
            -CurrentValue $CurrentValue -IsEdit $IsEdit -TypeNameFn $getTypeName -ShowMessage $showMessage
        ...
        try {
            ...
            & $showMessage -Title 'Error' -Message "Failed to write value: $($_.Exception.Message)"
        }
        finally {
            $parts = $path -split '\\', 2
            if ($null -ne $regKey -and $parts.Count -gt 1) { $regKey.Dispose() }
    ...
    $mnuValModify.Add_Click({
        ...
        try {
            ...
            & $showMessage -Title 'Error' -Message "Cannot read value: $($_.Exception.Message)"
        }
        finally {
            $parts = $path -split '\\', 2
            if ($null -ne $regKey -and $parts.Count -gt 1) { $regKey.Dispose() }
    ...
    $mnuValDelete.Add_Click({
        ...
        $path = $state.CurrentPath
        if (-not (& $confirm -Title 'Confirm Delete' -Message "Delete value '$name'?")) { return }
    ...
    $showFindDialog = {
        ...
        if ($hit) {
            ...
            & $showMessage -Title 'Find' -Message 'No match found.'
        }
    }.GetNewClosure()
...
'@
    ...
    $showValidationMessage = {
        ...
        $paramNames = @()
        try {
            $paramNames = @($ShowMessage.Ast.ParamBlock.Parameters | ForEach-Object { $_.Name.VariablePath.UserPath })
        } catch { }
        if ($paramNames -contains 'Owner') {
            & $ShowMessage -Title $Title -Message $Message -Owner $dlg
        }
        else {
    ...
    $btnOK.Add_Click({
        ...
        if (-not $IsEdit -and [string]::IsNullOrWhiteSpace($vName)) {
            & $showValidationMessage -Title 'Validation' -Message 'Value name cannot be empty.'
            return
        }
        try {
            $val = switch ($Kind) {
            ...
            & $showValidationMessage -Title 'Validation' -Message "Invalid value: $($_.Exception.Message)"
        }
    }.GetNewClosure())
...
function Show-RegImportPreview {
    ...
    $lblFile.FontFamily = 'Cascadia Code, Consolas, Courier New'
    ...
    $list.FontFamily = 'Cascadia Code, Consolas, Courier New'
    ...
    $warnBox.FontFamily = 'Cascadia Code, Consolas, Courier New'
