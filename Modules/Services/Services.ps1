...
function New-ServicesView {
    ...
    $loadServices = {
        ...
        try {
            ...
            & $showMessage -Title 'Error' -Message "Failed to load services: $($_.Exception.Message)"
        }
    ...
    $doServiceAction = {
        ...
        if ($Action -ne 'Start') {
            if (-not (& $confirm -Title 'Confirm' -Message "$Action service '$DisplayName' ($Name)?")) { return }
        }
        $view.Cursor = [System.Windows.Input.Cursors]::Wait
        try {
            switch ($Action) {
            ...
            & $showMessage -Title 'Error' -Message "Failed to $($Action.ToLower()) service '$Name': $($_.Exception.Message)"
